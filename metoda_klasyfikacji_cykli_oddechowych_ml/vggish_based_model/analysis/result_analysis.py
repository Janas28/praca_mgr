...
def analyse_error(clf, x_test, y_test, decision_function=False):
    ...
    c_matrix = confusion_matrix(y_test, y_test_predict, labels=[0,1])
    print("\n* * *\n")
    print("Confusion matrix: ")
    print(c_matrix)
    print("Rows - Actual values\nColumns - predicted values")
    print("Classes: 0 - cycle, 1 - silence or noise")
    ...
    if decision_function:
        ...
        precisions, recalls, thresholds = [0,0],[0,0],[0,0]
        fpr, tpr, thresholds_roc = [0,0],[0,0],[0,0]
        auc = [0,0]
        ...
        print("\n* * *\n")
        for i in range(num_class):
            class_name = "cycle" if i == 0 else "silence or noise"
