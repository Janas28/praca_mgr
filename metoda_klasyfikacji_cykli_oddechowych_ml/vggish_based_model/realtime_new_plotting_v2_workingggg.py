...
class AudioHandler(object):
    ...
    def plot_audio(self, curr_pred, last_audio, pred_tmstp, respiratory_cycle_counter, previous_pred_tmstp):
        ...
        if curr_pred == [0] and len(self.time_data) > 3:
            counter = 0
            for x in range(2, 3):
    ...
    def mainloop(self):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            while self.stream.is_active():
                # Pobierz ostatnie dane audio z bufora
                last_audio = np.array(self.buffer)

                # Ustaw pierwszy znacznik czasowy
                if first == 0:
                    first = time.perf_counter()

                # Oblicz bieżący znacznik czasowy predykcji
                pred_tmstp = time.perf_counter() - first

                # Przygotowanie danych do VGGish - wyciągnij ostatnią sekundę danych
                input_batch = vggish_input.waveform_to_examples(last_audio[-(self.RATE):], self.RATE)

                # Uzyskaj embeddingi z sesji TensorFlow
                ...
                # Funkcja pomocnicza do konwersji tf.data.Dataset na NumPy (jeśli konieczne)
                def dataset_to_numpy(dataset):
                    return np.array(list(dataset.as_numpy_iterator()))

                # Sprawdź, czy postprocessed_batch jest tf.data.Dataset i dokonaj konwersji
                if isinstance(postprocessed_batch, tf.data.Dataset):
                    postprocessed_batch = dataset_to_numpy(postprocessed_batch)

                # Wykonaj predykcję przy użyciu modelu na przetworzonych danych
                prediction = nn_model.predict(postprocessed_batch)

                # df = pd.DataFrame(postprocessed_batch)
                # print(df.values)

                # prediction = nn_model.predict(df.values[0])
